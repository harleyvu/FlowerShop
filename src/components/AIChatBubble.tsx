...
export default function AIChatBubble({ flowers }: { flowers: any[] }) {
  ...
  const [pendingSuggestion, setPendingSuggestion] = useState<{ product: any; qty: number } | null>(null);
  ...
  async function send() {
    ...
    const findFlowerInText = (txt: string) => {
      if (!txt) return null;
    ...
    // intent detection
    const isDirectAdd = /(?:thêm).*(?:giỏ|cart)|add.*to.*cart|thêm\s+hoa|thêm\s+vào\s+giỏ/i.test(userMsg.text);
    const isThankYou = /(^|\b)(cảm ơn|cam on|thank you|thanks|tnx|thx)(\b|!|\.|,)?/i.test(userMsg.text);
    const isAffirm = /(^|\b)(đồng ý|được|ok|okay|yes|có|đồngy)(\b|!|\.|,)?/i.test(userMsg.text);
    ...
    if (isDirectAdd) {
      ...
      if (flower) {
  addToCart({ productId: String(((flower.id ?? flower.productId ?? flower.productID ?? flower._id) || flower.name)), name: flower.name, price: Number(flower.price ?? flower.cost ?? 0) }, 1);
        // show same alert as ProductCard
        Alert.alert('Added to cart', `${flower.name} has added to cart`, [
          { text: 'Continue buy', style: 'cancel' },
          { text: 'Look at the cart', onPress: () => router.push({ pathname: '/cart' } as any) },
        ]);
        const aiMsg: Msg = { id: String(Date.now() + 5), sender: 'ai', text: `Đã thêm ${flower.name} vào giỏ hàng cho bạn.` };
        setMsgs((s) => [aiMsg, ...s]);
        return;
      }
    ...
    if (isThankYou) {
      ...
      if (suggested) {
        // set pending suggestion and ask for confirmation
        setPendingSuggestion({ product: suggested, qty: 1 });
        const aiMsg: Msg = { id: String(Date.now() + 3), sender: 'ai', text: `Cảm ơn bạn! Mình có thể thêm ${suggested.name} vào giỏ hàng cho bạn. Gõ "đồng ý" để xác nhận.` };
        setMsgs((s) => [aiMsg, ...s]);
        return;
      }
    ...
    // 3) If user affirms and we have a pending suggestion, add it
    if (isAffirm && pendingSuggestion) {
      const flower = pendingSuggestion.product;
  addToCart({ productId: String(((flower.id ?? flower.productId ?? flower._id) || flower.name)), name: flower.name, price: Number(flower.price ?? 0) }, pendingSuggestion.qty || 1);
      Alert.alert('Added to cart', `${flower.name} has added to cartỏ`, [
        { text: 'Continue buy', style: 'cancel' },
        { text: 'Look at the cart', onPress: () => router.push({ pathname: '/cart' } as any) },
      ]);
      const aiMsg: Msg = { id: String(Date.now() + 4), sender: 'ai', text: `Đã thêm ${flower.name} vào giỏ hàng cho bạn. Bạn muốn xem giỏ hàng không?` };
      setMsgs((s) => [aiMsg, ...s]);
